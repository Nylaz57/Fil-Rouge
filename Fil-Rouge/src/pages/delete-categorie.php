<?php
session_start();
if (isset($_SESSION['id']) && $_SESSION['statut'] === 4) {

    if (!empty($_POST['Id_famille'])) {
        require '../src/data/db-connect.php';

        $requete = $connexion->prepare("SELECT nom_famille FROM famille WHERE Id_famille = :Id_famille");
        $requete->execute([
            'Id_famille' => $_POST['Id_famille'],
        ]);
        $nom = $requete->fetch();

        $requete = $connexion->prepare("SELECT * FROM famille_modele WHERE Id_famille = :Id_famille");
        $requete->execute([
            'Id_famille' => $_POST['Id_famille'],
        ]);
        $modeles = $requete->fetchAll();

        // Foreign KEY en Cascade
        $requete = $connexion->prepare("DELETE FROM modele WHERE Id_modele = :Id_modele");
        foreach ($modeles as $modele) {
            $requete->execute([
                'Id_modele' => $modele['Id_modele'],
            ]);
        }

        $requete = $connexion->prepare("DELETE FROM famille WHERE Id_famille = :Id_famille");
        $requete->execute([
            'Id_famille' => $_POST['Id_famille'],
        ]);



        if ($nom) {
            $nomModif = str_replace(' ', '-', $nom['nom_famille']);
            $nomModif = strtolower($nomModif);
            $chemin = "assets/img/materiels/" . $nomModif;
            function supprimerDossier($dossier)
            {
                // Lire le contenu du dossier
                $fichiers =  array_diff(scandir($dossier), array('.', '..'));
                foreach ($fichiers as $fichier) {
                    // Si c'est un dossier, on appelle  la fonction
                    if (is_dir("$dossier/$fichier")) {
                        supprimerDossier("$dossier/$fichier");
                    } else {
                        // Si c'est un fichier, le supprimer
                        unlink("$dossier/$fichier");
                    }
                }
                // Supprimer le dossier vide
                return rmdir($dossier);
            }

            if (is_dir($chemin)) {
                (supprimerDossier($chemin));
            }

            header("Location: /?page=categories");
            die;
        }
    }
} elseif (isset($_SESSION['id']) && $_SESSION['statut'] != 4) {
    http_response_code(403);
    $titre = "Erreur 403";
    $page = 403;
} else {
    http_response_code(401);
    $titre = "Erreur 401";
    $page = 401;
}
